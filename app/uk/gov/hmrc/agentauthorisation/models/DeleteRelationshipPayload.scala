...
object DeleteRelationshipPayload {
  ...
  implicit val reads: Reads[DeleteRelationshipPayload] =
    ((JsPath \ "service").read[List[String]] and
      ...
      (JsPath \ "clientId").read[String].map(_.replaceAll(" ", "")))((service, clientType, clientIdType, clientId) =>
      DeleteRelationshipPayload(service, clientType, clientIdType, clientId)
    )
}
