...
object DeleteRelationshipPayload {
  ...
  implicit val reads: Reads[DeleteRelationshipPayload] =
    ((JsPath \ "service").read[List[String]] and
      ...
      (JsPath \ "agentType").readNullable[String])((service, clientType, clientIdType, clientId, agentType) =>
      DeleteRelationshipPayload(service, clientType, clientIdType, clientId, agentType)
    )
}
