...
object InvitationDetails {
  val dateTimeFormatter: DateTimeFormatter = DateTimeFormatter.ofPattern("yyyy-MM-dd'T'HH:mm:ss.nnn")
    ...
    details.status match {
      case "Pending" =>
        Json.obj(
          ...
          "created"         -> dateTimeFormatter.format(details.created),
          "expiresOn"       -> dateTimeFormatter.format(details.expiresOn),
          "arn"             -> arn.value,
      ...
      case _ =>
        Json.obj(
          "_links"  -> hrefJson(arn, InvitationId(details.invitationId)),
          "created" -> dateTimeFormatter.format(details.created),
          "updated" -> dateTimeFormatter.format(details.lastUpdated),
          "arn"     -> arn.value,
          "service" -> Seq(details.service.externalServiceName),
          "status"  -> details.status
        ) ++ agentTypeJson(details.service)
